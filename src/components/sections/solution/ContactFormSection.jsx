...
function mergeMembers(selected, fetched) {
  const fetchedById = new Map((fetched || []).map((item) => [Number(item.id), item]));
...
function getContactItems(member) {
  ...
  return {
    ...
    links: repeaterRows(acf.follow_me).map((row) => {
      const label = firstValue(row, [
        "social_media_lable",
        "social_media_label",
        "social_media_title",
...
export default function SolutionContactFormSection({
  data,
  teamData = null,
  lang,
